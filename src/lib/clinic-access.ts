...
export function matchesAccess(hospital: Pick<Hospital, 'accessEvidence'>, field: AccessField, department?: string | null): boolean {
...
export function reportedDepartmentMatches(rawName: string, department: string): boolean {
  ...
  return exact[department]?.includes(name) ?? false;
}
